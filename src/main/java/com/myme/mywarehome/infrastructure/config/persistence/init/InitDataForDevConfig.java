...
public class InitDataForDevConfig implements CommandLineRunner {
    ...
    private void initializeEngineProducts(List<Product> products, String engineType, String engineName, Map<String, Company> companyMap) {
        // Base engine
        addProduct(products, "10000", engineType, engineName, "엔진 완성품", companyMap.get("자체생산"), 1, 6);

        // 1. Cylinder Block Module
        addProduct(products, "10100", engineType, engineName, "실린더 블록 모듈", companyMap.get("자체생산"), 1, 6);
        addProduct(products, "10120", engineType, engineName, "실린더 블록", companyMap.get("포스코"), 1, 6);
        addProduct(products, "10130", engineType, engineName, "피스톤 모듈", companyMap.get("Mahle"), 4, 3);
        addProduct(products, "10140", engineType, engineName, "커넥팅 로드 모듈", companyMap.get("Mahle"), 4, 3);
        addProduct(products, "10150", engineType, engineName, "크랭크샤프트 모듈", companyMap.get("현대제철"), 1, 6);
        addProduct(products, "10200", engineType, engineName, "실린더 헤드 모듈", companyMap.get("Fel-Pro"), 1, 6);

        // 2. Fuel Supply Module
        Company fuelSupplyVendor = getFuelSupplyVendor(engineType, companyMap);
        addProduct(products, "60000", engineType, engineName, "연료공급 모듈", fuelSupplyVendor, 1, 3);

        // 3. Intake/Exhaust Module
        addProduct(products, "50000", engineType, engineName, "흡배기 모듈", companyMap.get("자체생산"), 1, 3);
        Company intakeVendor = getIntakeVendor(engineType, companyMap);
        addProduct(products, "50100", engineType, engineName, "흡기 매니폴드 모듈", intakeVendor, 1, 3);
        Company exhaustVendor = getExhaustVendor(engineType, companyMap);
        addProduct(products, "50200", engineType, engineName, "배기 매니폴드 모듈", exhaustVendor, 1, 3);

        // Add turbocharger for Gamma and Theta engines
        if (engineType.equals("04") || engineType.equals("06")) {
            addProduct(products, "50400", engineType, engineName, "터보차저 모듈", companyMap.get("BorgWarner"), 1, 6);
        }
        ...
        if (engineType.equals("06")) {
            addProduct(products, "20300", "06", engineName, "타이밍 모듈 (고성능)", companyMap.get("Schaeffler"), 1, 3);
        }
    }
    ...
    private void createCommonParts(List<Product> products, Map<String, Company> companyMap) {
        ...
        addProduct(products, "40100", "01", "Kappa/Gamma/Nu 공용", "워터펌프 어셈블리", companyMap.get("Aisin"), 1, 3);
        addProduct(products, "40210", "01", "Kappa/Gamma/Nu 공용", "서모스탯", companyMap.get("대원산업"), 1, 1);
        addProduct(products, "40300", "01", "Kappa/Gamma/Nu 공용", "냉각수 호스 세트", companyMap.get("동아튜브"), 1, 1);
        addProduct(products, "30310", "01", "Kappa/Gamma/Nu 공용", "오일 필터", companyMap.get("한국필터"), 1, 1);
        addProduct(products, "20300", "01", "Kappa/Gamma/Nu 공용", "타이밍 모듈", companyMap.get("Continental"), 1, 3);
    }
